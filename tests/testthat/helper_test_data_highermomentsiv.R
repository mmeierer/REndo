test.data.highermomentsiv <- function(){
  # response ~ endogenous | all exogenous | which to build internal IV | EIV
  # y ~ P | X1 + X2 | P
  test_that("Fail if not data.frame", {
    expect_error(higherMomentsIV(formula = y ~ P, data =     , g="x2", IIV="gp"), regexp = "The above errors were encountered!")
    expect_error(higherMomentsIV(formula = y ~ P, data = NULL, g="x2", IIV="gp"), regexp = "The above errors were encountered!")
    expect_error(higherMomentsIV(formula = y ~ P, data = NA_integer_, g="x2", IIV="gp"), regexp = "The above errors were encountered!")

    expect_error(higherMomentsIV(formula = y ~ P, data = c(y=1:10,   P=1:10), g="x2", IIV="gp"), regexp = "The above errors were encountered!")
    expect_error(higherMomentsIV(formula = y ~ P, data = list(y=1:10,P=1:10), g="x2", IIV="gp"), regexp = "The above errors were encountered!")
  })
