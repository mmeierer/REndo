...
Below, we present the syntax for each of the 6 implemented instrument-free methods:
...
### **Two-Stage Copula Generated Regressor**

```
tscope(y ~ X1 + X2 + P1 + P2 | P1 + P2, data)
```
The first argument is a two-part formula of the model to be estimated, where **y** is the response variable; **X1 + X2 + P1 + P2** represents the complete model to be estimated; and the second part **P1 + P2** specifies the endogenous regressors that will undergo the copula-based transformation. The exogenous regressors **X1** and **X2** are treated as strictly exogenous and are used in the residualization step if present. The method transforms endogenous regressors using their empirical cumulative distribution function followed by the inverse normal distribution, addressing endogeneity without requiring external instruments.
